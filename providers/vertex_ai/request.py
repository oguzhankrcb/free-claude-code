"""Request builder for Vertex AI provider."""
...
VERTEX_AI_DEFAULT_MAX_TOKENS = 81920
...
def build_request_body(request_data: Any) -> dict:
    """Build OpenAI-format request body from Anthropic request for Vertex AI."""
    logger.debug(
        "VERTEX_AI_REQUEST: conversion start model=%s msgs=%d",
    ...
    _set_if_not_none(body, "max_tokens", max_tokens or VERTEX_AI_DEFAULT_MAX_TOKENS)
    ...
    # Handle stream if set
    stream = getattr(request_data, "stream", None)
    if stream is not None:
        body["stream"] = stream

    logger.debug(
        "VERTEX_AI_REQUEST: conversion done model=%s msgs=%d tools=%d",
        body.get("model"),
